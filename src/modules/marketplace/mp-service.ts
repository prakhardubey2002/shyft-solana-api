...
export class MarketplaceService {
  ...
  async createMarketPlace(
    createMarketPlaceDto: CreateMarketplaceServiceDto,
  ): Promise<MarketPlaceResponseDto> {
    try {
      const creatorKp = AccountUtils.getKeypair(
        createMarketPlaceDto.createMarketplaceParams.private_key,
      );
      ...
      const metaplex = Metaplex.make(connection, {
        cluster: createMarketPlaceDto.createMarketplaceParams.network,
      });
      const auctionsClient = metaplex.auctions();

      const transactionFee =
        createMarketPlaceDto.createMarketplaceParams.transaction_fee ?? 2;
      const mpAuthority =
        createMarketPlaceDto.createMarketplaceParams.authority_address ??
        wallet.publicKey;
      const treasuryWithdrawalDestinationOwner = createMarketPlaceDto
        .createMarketplaceParams.fee_recipient
        ? new PublicKey(
            createMarketPlaceDto.createMarketplaceParams.fee_recipient,
          )
        : wallet.publicKey;
      ...
      const auctionHouse = await auctionsClient
        .createAuctionHouse({
          ...
          payer: wallet.payer,
          treasuryMint: createMarketPlaceDto.createMarketplaceParams
            .currency_address
            ? new PublicKey(
                createMarketPlaceDto.createMarketplaceParams.currency_address,
              )
            : NATIVE_MINT,
          authority: toPublicKey(mpAuthority),
          feeWithdrawalDestination: createMarketPlaceDto.createMarketplaceParams
            .fee_payer
            ? new PublicKey(
                createMarketPlaceDto.createMarketplaceParams.fee_payer,
              )
            : wallet.publicKey,
  ...
  async getMarketplaces(
    getMpDto: GetMpServiceDto,
  ): Promise<MarketplaceInfoResponseDto[]> {
    try {
      const data = await this.marketplaceRepo.getMarketplacesByApiKeyId(
        getMpDto.network,
        getMpDto.apiKeyId,
      );
      const result = data.map((d) => {
        const resp: MarketplaceInfoResponseDto = {
          ...
          fee_recipient: d.treasury_withdrawal_destination_owner,
          fee_recipient_account: d.treasury_withdrawal_destination,
  ...
  async findMarketplace(findMarketplaceDto: FindMarketplaceDto) {
    try {
      const auctionHousePda = findAuctionHousePda(
        toPublicKey(findMarketplaceDto.authority_address),
        toPublicKey(findMarketplaceDto.currency_address),
      );
      const auctionHouse = await Utility.auctionHouse.findAuctionHouse(
        findMarketplaceDto.network,
        auctionHousePda,
      );
      const currencySymbol = await Utility.token.getTokenSymbol(
        findMarketplaceDto.network,
        findMarketplaceDto.currency_address,
      );
      ...
      const resp: MarketPlaceResponseDto = {
        ...
        fee_recipient_account:
          auctionHouse.treasuryWithdrawalDestinationAddress.toBase58(),
        fee_recipient: treasuryDestinationOwner,
  ...
  async updateAuctionHouse(updateMarketplaceDto: UpdateMpSerivceDto) {
    try {
      const callerKp = AccountUtils.getKeypair(
        updateMarketplaceDto.update.private_key,
      );
      const connection = Utility.connectRpc(
        updateMarketplaceDto.update.network,
      );
      const metaplex = Metaplex.make(connection, {
        cluster: updateMarketplaceDto.update.network,
      });
      const auctionsClient = metaplex.auctions();
      const originalAuctionHouse = await auctionsClient
        .findAuctionHouseByAddress(
          new PublicKey(updateMarketplaceDto.update.marketplace_address),
        )
        .run();

      if (
        callerKp.publicKey.toBase58() !=
        originalAuctionHouse.authorityAddress.toBase58()
      ) {
        throw newProgramError(
          'updateAuctionHouse_Unauthorized_updater',
          HttpStatus.FORBIDDEN,
          'only the marketplace authority is allowed to update the marketplace',
          '',
          'mp-service_updateMarketplace',
          { updaterAddress: callerKp.publicKey.toBase58() },
        );
      }

      const treasuryDestinationOwner =
        updateMarketplaceDto.update.fee_recipient ??
        (await this.marketplaceRepo.getMarketplaceTreasuryDestinationOwner(
          updateMarketplaceDto.update.network,
          originalAuctionHouse.address.toBase58(),
        ));
      if (treasuryDestinationOwner === undefined) {
        throw newProgramError(
          ...
          {
            input_params: updateMarketplaceDto.update,
          },
        );
      }

      metaplex.use(keypairIdentity(callerKp));
      const { auctionHouse: updatedAuctionHouse } = await metaplex
        .auctions()
        .updateAuctionHouse(originalAuctionHouse, {
          sellerFeeBasisPoints: updateMarketplaceDto.update.transaction_fee
            ? updateMarketplaceDto.update.transaction_fee * 100
            : originalAuctionHouse.sellerFeeBasisPoints,
          newAuthority: updateMarketplaceDto.update.new_authority_address
            ? toPublicKey(updateMarketplaceDto.update.new_authority_address)
            : originalAuctionHouse.authorityAddress,
          feeWithdrawalDestination: updateMarketplaceDto.update.fee_payer
            ? toPublicKey(updateMarketplaceDto.update.fee_payer)
            : originalAuctionHouse.feeWithdrawalDestinationAddress,
      ...
      const updatedTransactionFee =
        updatedAuctionHouse.sellerFeeBasisPoints / 100;
      const currencySymbol = await Utility.token.getTokenSymbol(
        updateMarketplaceDto.update.network,
    ...
    try {
      ...
      if (
        ...
        throw newProgramError(
          ...
          { withdrawer_address: callerKp.publicKey.toBase58() },
        );
      }
  ...
  async getTreasuryBalance(getTreasuryBalance: GetTreasuryBalanceDto) {
    try {
      const marketplace = await this.marketplaceRepo.getMarketplacesByAddress(
        getTreasuryBalance.network,
        getTreasuryBalance.marketplace_address,
      );
      ...
      if (marketplace) {
        ...
        const metaplex = Metaplex.make(connection, {
          cluster: getTreasuryBalance.network,
        });
        const auctionsClient = metaplex.auctions();
        const auctionHouse = await auctionsClient
          .findAuctionHouseByAddress(
            new PublicKey(getTreasuryBalance.marketplace_address),
          )
          .run();
      ...
      currencySymbol =
        currencySymbol ??
        (await Utility.token.getTokenSymbol(
          getTreasuryBalance.network,
          currencyAddress,
        ));
