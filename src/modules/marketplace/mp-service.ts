...
import { MarketplaceCreationEvent, UpdateMarketplaceEvent } from '../data-cache/db-sync/db.events';
...
export class MarketplaceService {
  ...
  async createMarketPlace(createMarketPlaceDto: CreateMarketplaceServiceDto): Promise<MarketPlaceResponseDto> {
    try {
      ...
      const mpAuthority = createMarketPlaceDto.createMarketplaceParams.authority_address ?? wallet.publicKey;
      const treasuryWithdrawalDestinationOwner = createMarketPlaceDto.createMarketplaceParams.fee_recipient
        ? new PublicKey(createMarketPlaceDto.createMarketplaceParams.fee_recipient)
        : wallet.publicKey;
      ...
      const auctionHouse = await auctionsClient
        .createAuctionHouse({
          ...
          payer: wallet.payer,
          treasuryMint: createMarketPlaceDto.createMarketplaceParams.currency_address
            ? new PublicKey(createMarketPlaceDto.createMarketplaceParams.currency_address)
            : NATIVE_MINT,
          authority: toPublicKey(mpAuthority),
          feeWithdrawalDestination: createMarketPlaceDto.createMarketplaceParams.fee_payer
            ? new PublicKey(createMarketPlaceDto.createMarketplaceParams.fee_payer)
            : wallet.publicKey,
  ...
  async updateAuctionHouse(updateMarketplaceDto: UpdateMpSerivceDto) {
    try {
      ...
      const originalAuctionHouse = await auctionsClient
        .findAuctionHouseByAddress(new PublicKey(updateMarketplaceDto.update.marketplace_address))
        .run();
  ...
  async getTreasuryBalance(getTreasuryBalance: GetTreasuryBalanceDto) {
    try {
      ...
      if (marketplace) {
        ...
        const auctionsClient = metaplex.auctions();
        const auctionHouse = await auctionsClient
          .findAuctionHouseByAddress(new PublicKey(getTreasuryBalance.marketplace_address))
          .run();
      ...
      currencySymbol =
        currencySymbol ?? (await Utility.token.getTokenSymbol(getTreasuryBalance.network, currencyAddress));
