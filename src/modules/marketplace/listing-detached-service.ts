...
export class ListingDetachedService {
    ...
    try {
      ...
      const auctionHouse = await auctionsClient
        .findAuctionHouseByAddress(
          toPublicKey(createListDto.params.marketplace_address),
        )
        .run();
      ...
      const offerPrice = amount(
        toBigNumber(
          createListDto.params.price * Math.pow(10, auctionCurrency.decimals),
        ),
      ...
      const printReceiptInstruction = createPrintListingReceiptInstruction(
        printAccounts,
        pArgs,
      );
  ...
  async buy(buyDto: BuyDto): Promise<any> {
    try {
      ...
      const auctionsClient = metaplex.auctions();
      const auctionHouse = await auctionsClient
        .findAuctionHouseByAddress(toPublicKey(buyDto.marketplace_address))
        .run();
      ...
      const printPurchaseReceiptInstrction =
        createPrintPurchaseReceiptInstruction(psAccounts, psArgs);
