import { findAssociatedTokenAccountPda, findAuctionHouseFeePda, findAuctionHousePda, findAuctionHouseTreasuryPda, Metaplex, toBigNumber, toPublicKey } from "@metaplex-foundation/js";
import { CreateAuctionHouseInstructionAccounts, CreateAuctionHouseInstructionArgs, createCreateAuctionHouseInstruction, createUpdateAuctionHouseInstruction, createWithdrawFromTreasuryInstruction, UpdateAuctioneerInstructionArgs, UpdateAuctionHouseInstructionAccounts, UpdateAuctionHouseInstructionArgs, WithdrawFromTreasuryInstructionAccounts } from "@metaplex-foundation/mpl-auction-house";
...
import { MarketplaceInitiationEvent, MarketplaceUpdateInitiationEvent } from "../helper/db-sync/db.events";
...
export class MarketplaceDetachedService {
	...
	async createMarketPlace(createMarketPlaceDto: CreateMarketplaceServiceDto) {
		try {
			...
			// Accounts.
			const authority = toPublicKey(createMarketPlaceDto.params.creator_wallet);
			const treasuryMint = createMarketPlaceDto.params.currency_address ? toPublicKey(createMarketPlaceDto.params.currency_address) : NATIVE_MINT;
			const treasuryWithdrawalDestinationOwner = toPublicKey(createMarketPlaceDto.params.fee_recipient ?? createMarketPlaceDto.params.creator_wallet);
			const feeWithdrawalDestination = toPublicKey(createMarketPlaceDto.params.fee_payer ?? createMarketPlaceDto.params.creator_wallet);
			...
			const treasuryWithdrawalDestination = treasuryMint.equals(NATIVE_MINT) ? treasuryWithdrawalDestinationOwner : findAssociatedTokenAccountPda(treasuryMint, treasuryWithdrawalDestinationOwner);
			const currencySymbol = await Utility.token.getTokenSymbol(createMarketPlaceDto.params.network, treasuryMint.toBase58());
			...
			this.eventEmitter.emit('marketplace.creation.initiated', new MarketplaceInitiationEvent(
	...
	async updateMarketplace(updateMarketplaceDto: UpdateMpSerivceDto) {
		try {
			...
			const auctionHouse = await Utility.auctionHouse.findAuctionHouse(updateMarketplaceDto.params.network, auctionHouseAddress);

			let treasuryWithdrawalDestinationOwner: PublicKey;
			let treasuryWithdrawalDestination: PublicKey;

			if (auctionHouse.isNative) {
				treasuryWithdrawalDestinationOwner = updateMarketplaceDto.params.fee_recipient ? toPublicKey(updateMarketplaceDto.params.fee_recipient) : auctionHouse.treasuryWithdrawalDestinationAddress;
				...
				throw newProgramError("missing_params", HttpStatus.BAD_REQUEST,
			...
			if (!shouldSendUpdateInstruction) {
				throw newProgramError("no_update_error", HttpStatus.BAD_REQUEST,
	...
	async withdrawFee(withdrawRoyaltyDto: WithdrawFeeDto): Promise<any> {
		try {
			...
			if (executor.toBase58() != auctionHouse.authorityAddress.toBase58()) {
				throw newProgramError("withdrawFee_Unauthorized_withdrawal", HttpStatus.FORBIDDEN, "only the marketplace authority is allowed to withdraw funds from the marketplace", "", "mp-service_withdrawFee", {
