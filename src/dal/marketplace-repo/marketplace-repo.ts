...
export class MarketplaceRepo {
  constructor(
    ...
    public MarketplaceModel: Model<MarketplaceDocument>,
  ) {}

  public async insert(data: Marketplace): Promise<any> {
    try {
      const filter = { network: data.network, address: data.address };
      const result = await this.MarketplaceModel.updateOne(filter, data, {
        upsert: true,
      });
      return result;
    } catch (err) {
  ...
  public async getMarketplacesByApiKeyId(
  ...
  public async getMarketplacesByAddress(
    network: WalletAdapterNetwork,
    address: string,
  ): Promise<MarketplaceDocument> {
    try {
  ...
  public async getMarketplaceTreasuryDestinationOwner(
    network: WalletAdapterNetwork,
    mpAddress: string,
  ): Promise<string> {
    try {
      const filter = { network: network, address: mpAddress };
      const result = await this.MarketplaceModel.findOne(filter);
      if (result != null) {
        return result.treasury_withdrawal_destination_owner;
      }
      return null;
    } catch (err) {
      throw new Error(err);
